...
export default function Activate(){
  ...
  if(latest&&report&&review&&score!==null){
    return <main className="container section onboarding-page"><Wordmark/><div style={{maxWidth:980,margin:'34px auto 70px',display:'grid',gap:16}}>
      <section className="glass card" style={{padding:'clamp(24px,4vw,46px)',border:'1px solid rgba(182,246,107,.25)'}}><div className="eyebrow">WE FOUND YOUR FIRST FOCUS</div><h1 style={{fontSize:'clamp(38px,6vw,68px)',lineHeight:.95,margin:'12px 0 14px'}}>{review.biggestMistake.title}</h1><p style={{fontSize:18,lineHeight:1.6,maxWidth:820}}>{report.primary.inference}</p><div className="grid three" style={{marginTop:20}}><div><span className="label">THE GAME</span><b>{latest.champion} · {latest.result} · {latest.kills}/{latest.deaths}/{latest.assists}</b></div><div><span className="label">WHAT WE SAW</span><b>{report.primary.facts[1]||report.primary.facts[0]}</b></div><div><span className="label">EVIDENCE</span><b>{proAnalysis?'Riot timeline + game evidence':'Scoreboard starting point'}</b></div></div></section>
      ...
      <section ref={nextStepsRef} className="glass card"><div className="eyebrow">WHAT HAPPENS NOW</div><h2>Do not try to fix everything.</h2><div className="grid three" style={{marginTop:18}}><div><span className="label">01 · PLAY</span><b>Take that one rule into your next game.</b></div><div><span className="label">02 · CHECK</span><b>We look for the same behaviour again.</b></div><div><span className="label">03 · MOVE ON</span><b>When it improves, your Coach gives you the next focus.</b></div></div></section>
  ...
  return <main className="container section onboarding-page"><Wordmark/><div style={{maxWidth:860,margin:'34px auto 70px',display:'grid',gap:16}}>
    ...
    {syncStatus?.enabled&&!syncError&&<section className="glass card"><div className="eyebrow">LATEST RANKED GAME</div><h2>{syncing?'Getting your latest game…':'Your Riot account is connected.'}</h2><p className="muted">One game is enough to give you a useful starting focus. We’ll learn more as you play.</p>{!syncing&&<button className="btn primary" onClick={()=>void runSync()}>GET MY LATEST GAME</button>}</section>}
