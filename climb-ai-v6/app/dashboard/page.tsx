  ...
  return <AppShell>
    ...
    <div className="v7-stack">
      ...
      <section className="hq-command">
        <div className="hq-top"><span className="v7-badge engine">{detail.tier} COACH</span><span className="v7-badge">THIS WEEK&apos;S FOCUS</span></div>
        ...
        {cleanStartStreak>=2&&<div className="focus-streak"><b>🔥 {cleanStartStreak} CLEAN STARTS IN A ROW</b><span>0 deaths before 10:00 in each of your latest {cleanStartStreak} tracked games.</span></div>}

        {leadTask&&<IlpExplainability task={leadTask} compact/>}

        {leadTask&&leadEvidence&&detail.depth>=2&&<div className="rep" style={{marginTop:14}}>
          <div className="eyebrow">WHAT WE SAW</div>
          <b className="rep-headline">{leadEvidence.available?'This came from your game.':'We need more proof.'}</b>
          <p className="rep-detail">{leadEvidence.reason}</p>
        </div>}
    ...
    {activeTasks.length>1&&<section className="v7-section">
      <div className="v7-section-head"><div><div className="eyebrow">IN THE BACKGROUND</div><h2>You do not need to fix five things at once.</h2><p className="muted">These are being watched quietly. Your first focus stays first until the evidence says otherwise.</p></div><Link className="v7-link" href="/ilp">SEE MY PROGRESS →</Link></div>
