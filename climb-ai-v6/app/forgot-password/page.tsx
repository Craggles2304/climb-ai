...
export default function ForgotPassword(){
  ...
  return <main className="container section">
    ...
    <div className="glass card form" style={{margin:'55px auto'}}>
      ...
      <p className="muted">Enter the email address on your OP CLIMB account. We’ll send you a secure reset link.</p>
      ...
      {!sent?<form onSubmit={submit}>
        ...
        If an OP CLIMB account exists for <strong>{email}</strong>, a password-reset email has been sent. Open the newest email and follow the link.
      </div>}

      {error&&<div className="auth-message" role="alert">{error}</div>}

      <div className="divider"/>
      <p className="muted"><Link href="/login" className="text-link">← BACK TO LOG IN</Link></p>
    </div>
  </main>;
}
