...
export default function Analysis(){
  ...
  return <AppShell>
    ...
    <div className="glass card" style={{marginTop:18}}><div className="eyebrow">NEXT GAME</div><h2>{report.mission.title}</h2>{report.mission.rules.slice(0,detail.depth<=1?1:detail.depth<=2?2:3).map((rule,index)=><div className="cue-row" key={rule}><span>{['WHEN','DO','CHECK'][index]??'RULE'}</span><b>{rule}</b></div>)}{detail.depth>=3&&<p className="muted">Pass condition: {report.mission.target} {report.mission.unit} across {report.mission.gamesRequired} relevant games.</p>}<p className="muted">{existingTask?'This review matches one of your active measurable missions. OP CLIMB updates it automatically from the finished game.':'Your two-mission plan only changes when repeated measurable evidence is strong enough to justify a new focus.'}</p><Link href="/ilp" className="btn primary">OPEN DEVELOPMENT PLAN</Link></div>
