...
export default function Signup(){
  ...
  const submit=async(e:React.FormEvent)=>{e.preventDefault();setError('');setBusy(true);track('signup_started',{});try{const user=await authService.signUp(email,password);if(user.id==='pending'){setCheck(true);return}router.push('/onboarding');router.refresh()}catch(err){setError(err instanceof AuthNotConfiguredError?err.message:err instanceof Error?err.message:'Something went wrong.')}finally{setBusy(false)}};
  ...
  return <><main className="container section"><Wordmark/><div className="glass card form" style={{margin:'55px auto'}}><div className="eyebrow">START THE HUNT</div><h1>Create account</h1>
    ...
    {configured&&<form onSubmit={submit}><label className="field"><span>Email</span><input className="input" type="email" autoComplete="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com"/></label><label className="field"><span>Password</span><input className="input" type="password" autoComplete="new-password" required minLength={8} value={password} onChange={e=>setPassword(e.target.value)}/><small className="muted" style={{fontSize:11}}>At least eight characters.</small></label><div className="login-actions"><button className="btn primary" type="submit" disabled={busy||!configured}>{busy?'CREATING…':'CREATE ACCOUNT'}</button><button className="btn google-btn" type="button" disabled={busy||!configured} onClick={()=>authService.signInWithGoogle('/onboarding').catch(e=>setError(e.message))}><span className="google-g">G</span> CONTINUE WITH GOOGLE</button></div></form>}
    {error&&<div className="auth-message" role="alert">{error}</div>}
    <p className="muted">Start free. Your first goal is to generate one useful next-game mission.</p><p className="muted" style={{fontSize:12}}>Already have an account? <Link href="/login" className="text-link">LOG IN →</Link></p><p className="muted" style={{fontSize:12}}>Want to see the product first? <Link href="/demo" className="text-link">OPEN DEMO →</Link></p>
