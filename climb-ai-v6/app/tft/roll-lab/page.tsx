...
export default function TftRollLab(){
  ...
  const curve=useMemo(()=>[10,20,30,40,50,60].map(g=>({gold:g,current:estimateRoll({...shared,goldBudget:g}),next:level<10?estimateRoll({...shared,level:level+1,goldBudget:g}):null})),[level,cost,goal,own,contested,otherTierOut]);
  ...
  return <TftShell><main className="container section">
    ...
    <section className="glass card" style={{marginTop:18}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:10}}><a className="btn secondary" target="_blank" rel="noreferrer" href="https://teamfighttactics.leagueoflegends.com/en-us/news/game-updates/teamfight-tactics-patch-18-2/">RIOT 18.2 NOTES</a><a className="btn secondary" target="_blank" rel="noreferrer" href="https://tftflow.com/tables/set18/shop-odds-pool-size-xp-table">SHOP / POOL TABLE</a></div>
    </section>
    ...
    <section className="glass card" style={{marginTop:16}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(230px,1fr))',gap:12}}>
        <div style={{padding:14,borderRadius:16,background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.09)'}}><div className="eyebrow">ROLL NOW · LEVEL {level}</div><h1 style={{margin:'5px 0'}}>{pct(rollNow.chanceFinish)}</h1><b>finish {goal}★ with {comparison.rollNowBudget}g total spend budget</b><div className="cue-row"><span>AT LEAST ONE PURCHASED TARGET</span><b>{pct(rollNow.chanceAtLeastOne)}</b></div><div className="cue-row"><span>EXPECTED TARGETS BOUGHT</span><b>{rollNow.expectedHits.toFixed(2)}</b></div><div className="cue-row"><span>EXPECTED GOLD SPENT</span><b>{rollNow.expectedGoldSpent.toFixed(1)}g</b></div><div className="cue-row"><span>{cost}-COST ODDS / SLOT</span><b>{pct(rollNow.tierOdds)}</b></div></div>
        {comparison.levelFirst&&<div style={{padding:14,borderRadius:16,background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.09)'}}><div className="eyebrow">LEVEL FIRST · LEVEL {level+1}</div><h1 style={{margin:'5px 0'}}>{pct(comparison.levelFirst.chanceFinish)}</h1><b>{comparison.levelCost}g XP · {comparison.levelFirstBudget}g total spend budget left</b><div className="cue-row"><span>AT LEAST ONE PURCHASED TARGET</span><b>{pct(comparison.levelFirst.chanceAtLeastOne)}</b></div><div className="cue-row"><span>EXPECTED TARGETS BOUGHT</span><b>{comparison.levelFirst.expectedHits.toFixed(2)}</b></div><div className="cue-row"><span>EXPECTED GOLD SPENT</span><b>{comparison.levelFirst.expectedGoldSpent.toFixed(1)}g</b></div><div className="cue-row"><span>{cost}-COST ODDS / SLOT</span><b>{pct(comparison.levelFirst.tierOdds)}</b></div></div>}
    ...
    <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.2fr) minmax(250px,.8fr)',gap:14,marginTop:16}}>
      <div className="glass card"><div className="eyebrow">PROBABILITY CURVE</div><h2>HOW MUCH TOTAL SPEND CHANGES THE HIT</h2><div style={{display:'grid',gap:9}}>{curve.map(row=><div key={row.gold}><div style={{display:'grid',gridTemplateColumns:'54px 1fr auto',gap:8,alignItems:'center'}}><b>{row.gold}g</b><div style={{height:9,borderRadius:99,background:'rgba(255,255,255,.07)',overflow:'hidden'}}><div style={{height:'100%',width:`${Math.round(row.current.chanceFinish*100)}%`,background:'currentColor'}}/></div><b>{pct(row.current.chanceFinish)}</b></div>{row.next&&<div className="muted" style={{fontSize:10,marginLeft:62}}>Same {row.gold}g total spend budget at level {level+1}: {pct(row.next.chanceFinish)}</div>}</div>)}</div></div>
      <div className="glass card"><div className="eyebrow">GOLD TARGETS · LEVEL {level}</div><h2>ROLLOUT THRESHOLDS</h2><div className="cue-row"><span>50% TO FINISH</span><b>{thresholds.p50===null?'160g+':`${thresholds.p50}g`}</b></div><div className="cue-row"><span>80% TO FINISH</span><b>{thresholds.p80===null?'160g+':`${thresholds.p80}g`}</b></div><div className="cue-row"><span>95% TO FINISH</span><b>{thresholds.p95===null?'160g+':`${thresholds.p95}g`}</b></div><p className="muted" style={{fontSize:11}}>Thresholds include both 2g refreshes and the gold needed to purchase each target copy. They remain estimates because the rest of the lobby can change the pool during a real rolldown.</p></div>
    </section>
    ...
    <section className="glass card" style={{marginTop:16}}><div className="eyebrow">MODEL BOUNDARY</div><p className="muted" style={{margin:0}}>Set 18 / Patch 18.2 shop odds and bag sizes are system inputs. Every paid refresh costs 2g and exposes five shop slots; every target copy successfully bought also removes its unit cost from the entered spend budget. The model updates target depletion as copies are purchased and assumes entered contested/other same-cost copies stay out of the pool. It does not model future opponent purchases, Wisps, special Augments, duplicators, Loaded Dice-style effects or other non-standard shop modifiers.</p><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:10}}>{([1,2,3,4,5] as TftCost[]).map(c=><span key={c} className="op-tier op-tier-free">{c}-COST · {TFT_BAG_SIZE[c]} EACH · {TFT_UNIQUE_BY_COST[c]} UNITS</span>)}</div></section>
  </main></TftShell>;
}
