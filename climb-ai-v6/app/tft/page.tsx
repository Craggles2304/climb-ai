...
export default function TftHome(){
  ...
  return <TftShell><main className="container section">
    ...
    <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12,marginTop:16}}>
      ...
      <Link href="/tft/item-finder" className="glass card" style={{textDecoration:'none'}}><div className="eyebrow">LEGACY SET 17 ITEM SNAPSHOT</div><h3>COMPONENT → CARRY PATHS</h3><p className="muted">The item-fit engine remains usable as a mechanic demo, but its seeded carry-performance profiles need a Set 18 refresh before being treated as current meta.</p></Link>
      <Link href="/tft/carry-builder" className="glass card" style={{textDecoration:'none'}}><div className="eyebrow">LEGACY SET 17 CARRY SNAPSHOT</div><h3>CARRY + TEAM SHELL BUILDER</h3><p className="muted">Board-shell generation still uses current Riot static units; the seeded performance and item recommendations are explicitly legacy until refreshed for Set 18.</p></Link>
    ...
    <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.5fr) minmax(260px,.7fr)',gap:16,marginTop:18}}>
      ...
      <div className="glass card"><div className="eyebrow">NO-KEY TOOLKIT</div><h3>STATIC + SCENARIO TOOLS</h3><p className="muted">Current system math and Riot static data do not require a Riot match API key.</p><Link className="btn primary" href="/tft/roll-lab">OPEN ROLL LAB</Link><Link className="btn secondary" href="/tft/board-lab" style={{marginTop:8}}>OPEN BOARD LAB</Link><Link className="btn secondary" href="/tft/set-lab" style={{marginTop:8}}>OPEN SET LAB</Link><Link className="btn secondary" href="/tft/coach" style={{marginTop:8}}>OPEN TFT COACH</Link></div>
    </section>
  </main></TftShell>;
}
