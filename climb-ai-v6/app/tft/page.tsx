...
export default function TftHome(){
  ...
  return <TftShell><main className="container section">
    ...
    <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12,marginTop:16}}>
      ...
      <Link href="/tft/item-finder" className="glass card" style={{textDecoration:'none'}}><div className="eyebrow">ITEM-FIRST FLEX ENGINE</div><h3>TURN MY COMPONENTS INTO CARRY OPTIONS</h3><p className="muted">Enter components and completed items, then rank the three strongest carry directions and the safest flexible slams.</p></Link>
      <Link href="/tft/carry-builder" className="glass card" style={{textDecoration:'none'}}><div className="eyebrow">META CORE + ITEM BUILDER</div><h3>CHOOSE THE CHAMPION TO BUILD AROUND</h3><p className="muted">See strongest current carry cores, real item packages, alternatives and fun lines like triple-Rageblade Jax, then generate a team shell.</p></Link>
    ...
    <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.5fr) minmax(260px,.7fr)',gap:16,marginTop:18}}>
      ...
      <div className="glass card"><div className="eyebrow">NO-KEY TOOLKIT</div><h3>STATIC DATA STILL WORKS</h3><p className="muted">Champions, items, augments, traits and trait breakpoints can come from Riot Data Dragon without a match API key.</p><Link className="btn primary" href="/tft/item-finder">OPEN ITEM FINDER</Link><Link className="btn secondary" href="/tft/carry-builder" style={{marginTop:8}}>OPEN CARRY BUILDER</Link><Link className="btn secondary" href="/tft/board-lab" style={{marginTop:8}}>OPEN BOARD LAB</Link><Link className="btn secondary" href="/tft/set-lab" style={{marginTop:8}}>OPEN SET LAB</Link><Link className="btn secondary" href="/tft/coach" style={{marginTop:8}}>OPEN TFT COACH</Link></div>
    </section>
  </main></TftShell>;
}
