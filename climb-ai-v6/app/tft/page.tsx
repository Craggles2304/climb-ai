...
export default function TftHome(){
  ...
  return <TftShell><main className="container section">
    ...
    <section className="glass card" style={{marginTop:16,padding:14}}><div className="eyebrow">LIVE SET STATUS</div><b>Set 18 · Enchanted Wilds · Patch 18.2 is the current system baseline.</b><p className="muted" style={{margin:'4px 0 0'}}>Roll Lab uses current Set 18 economy rules, while Carry Builder and Item Finder now use the current Patch 18.2 champion-performance snapshot with standard craftable item packages separated from fun/high-roll lines.</p></section>
    ...
    <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.5fr) minmax(260px,.7fr)',gap:16,marginTop:18}}>
      ...
      <div className="glass card"><div className="eyebrow">NO-KEY TOOLKIT</div><h3>STATIC + SCENARIO TOOLS</h3><p className="muted">Current system math and Riot static data do not require a Riot match API key.</p><Link className="btn primary" href="/tft/roll-lab">OPEN ROLL LAB</Link><Link className="btn secondary" href="/tft/item-finder" style={{marginTop:8}}>OPEN ITEM FINDER</Link><Link className="btn secondary" href="/tft/carry-builder" style={{marginTop:8}}>OPEN CARRY BUILDER</Link><Link className="btn secondary" href="/tft/board-lab" style={{marginTop:8}}>OPEN BOARD LAB</Link><Link className="btn secondary" href="/tft/coach" style={{marginTop:8}}>OPEN TFT COACH</Link></div>
    </section>
  </main></TftShell>;
}