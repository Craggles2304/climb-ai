...
function AbilityPanel({side,label,showMath}:{side:SideView;label:string;showMath:boolean}){
  return <div className="glass card"><div className="section-row"><div><div className="eyebrow">{label}</div><h2 style={{margin:'6px 0 0'}}>{side.name}</h2></div><span className={`conf-tag ${CONFIDENCE_CLASS[side.confidence.level]}`}>{side.confidence.level}</span></div>
    <div className="spike-list" style={{marginTop:14}}>{side.abilities.map(a=>{
      const resolvedDamage=a.damage.filter(d=>d.raw!==null&&d.raw!==undefined);
      const damageLine=resolvedDamage.length
        ?resolvedDamage.map(d=>`${d.raw} ${d.type.toLowerCase()}`).join(' + ')+' damage'
        :'No direct damage figure';
      return <div className="spike" key={a.slot}><div className="spike-level">{a.slot}</div><div><b>{a.name} <span className="muted">rank {a.rank}/{a.maxRank}</span>{resolvedDamage.length>1&&<span className="tag-chip live-pill" style={{marginLeft:7}}>MIXED · {resolvedDamage.length} COMPONENTS</span>}</b><p className="spike-fact">{damageLine} · {a.cooldownSeconds}s · {a.cost>0?`${a.cost} resource`:'no cost'}{a.rangeUnits?` · ${a.rangeUnits} range`:''}</p>{resolvedDamage.length>1&&<div className="tag-row" style={{marginTop:6}}>{resolvedDamage.map((d,i)=><span className="tag-chip" key={`${d.label}-${i}`}>{d.label}: <b>{d.raw}</b> {d.type.toLowerCase()}</span>)}</div>}{a.confidence.level!=='HIGH'&&<p className={`conf-inline ${CONFIDENCE_CLASS[a.confidence.level]}`}>{a.confidence.level}: {a.confidence.causes[0]?.reason}</p>}{showMath&&a.calculations.length>0&&<table className="table skill-grid" style={{marginTop:8}}><tbody>{a.calculations.map(c=><tr key={c.name}><td style={{textAlign:'left'}}>{c.primary?'▸ ':''}{c.name}</td><td>{c.value??'—'}</td><td style={{textAlign:'left',color:'var(--muted)',fontSize:10}}>{c.primary?'Included in cast total':c.unmodelled[0]??'Not included: variant / auxiliary calculation'}</td></tr>)}</tbody></table>}</div></div>;
    })}</div>
  </div>;
}
