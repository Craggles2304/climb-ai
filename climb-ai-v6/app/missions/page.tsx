...
export default function Missions(){
  ...
  const activeTwo=tasks.filter(task=>task.status!=='MASTERED'&&task.status!=='PAUSED').slice(0,2);
  const mastered=tasks.filter(task=>task.status==='MASTERED').length;

  return <AppShell>
    <PageHead title="Mission Lab" subtitle={`${active.gameName}${active.tagline} · ${active.rank} · ${detail.tier} MISSIONS ${detail.depth}/10`} action={<Link href="/ilp" className="btn secondary">MY TWO MISSIONS</Link>}/>
    ...
    <section className="glass card" style={{marginBottom:18,borderColor:'rgba(182,246,107,.22)'}}>
      ...
      <p className="muted" style={{maxWidth:820,lineHeight:1.6}}>{detail.depth<=2?'Your missions are the two measurable habits OP CLIMB wants you to practise. Play normally and let your finished games prove when each one is ready to move on.':'Each mission has a number OP CLIMB can read from the finished game. Repeated proven games build mastery automatically.'}</p>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:14}}><span className="op-tier op-tier-pro">{activeTwo.length} ACTIVE</span>{detail.depth>=3&&<span className="op-tier op-tier-plus">{mastered} MASTERED</span>}{detail.depth>=4&&<span className="op-tier op-tier-plus">{last?`LAST GAME: ${last.champion} ${last.result}`:'WAITING FOR MATCH EVIDENCE'}</span>}</div>
    </section>

    {activeTwo.length===0?<section className="glass card"><div className="eyebrow">WAITING FOR PLAN</div><h2>No active missions yet.</h2><p className="muted">Track a game and OP CLIMB will build your first evidence-backed priorities.</p></section>:
    <div style={{display:'grid',gap:16}}>{activeTwo.map((task,index)=>{
      ...
      return <section key={task.id} className="glass card" style={{borderColor:index===0?'rgba(182,246,107,.34)':'var(--border)',padding:22}}>
        ...
        {detail.depth>=3&&(last&&evidence.available?<BehaviourCheck matchId={last.id} taskId={task.id} behaviour={task.gameRule} clearedBar={evidence.clearedBar} onResult={attempt=>recordMissionResult(task.id,attempt)}/>:<div style={{padding:14,border:'1px solid rgba(182,246,107,.2)',background:'rgba(182,246,107,.05)'}}><div className="eyebrow">WAITING FOR EVIDENCE</div><p className="muted" style={{margin:'6px 0 0'}}>{evidence.reason}</p></div>)}
      </section>;
    })}</div>}

    <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:18}}><Link className="btn primary" href="/live">PLAY + TRACK</Link><Link className="btn secondary" href="/coach">ASK {detail.tier} COACH</Link><Link className="btn secondary" href="/ilp">MY TWO MISSIONS</Link>{detail.depth>=3&&<Link className="btn secondary" href="/analyse">REVIEW LAST GAME</Link>}</div>
  </AppShell>;
}
