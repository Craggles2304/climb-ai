...
describe('adaptive PRO ILP',()=>{
 ...
 it('keeps the role plan capped at three active missions',()=>{const result=ensureFiveActive([], [game('1',42),game('2',48),game('3',82)],'a','MID','Gold');expect(result.tasks.filter(t=>t.status!=='MASTERED'&&t.status!=='PAUSED')).toHaveLength(3)});
});
