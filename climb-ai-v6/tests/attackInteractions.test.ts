...
test('Galio Colossal Smash uses current 16.18 replacement scaling and removes the normal auto',()=>{
  ...
  assert.equal(replacement?.attackSpeedFlatWhileReady,.25);
  ...
  // Ready Colossal Smash goes through MR: 325 -> 162.5. Its ready-state +0.25 attacks/s
  // moves the next action to t=.8; that second attack is ordinary physical damage: 100 -> 50.
  assert.equal(sim.events[0].mitigatedDamage,162.5);
  assert.equal(sim.events[1].atSeconds,.8);
  assert.equal(sim.events[1].mitigatedDamage,50);
});

test('Galio passive models recharge and ready-state attack speed while keeping crit explicit',()=>{
