...
test('Progress, API and learning snapshot expose one evidence-gated Curriculum',()=>{
  ...
  assert.ok(route.includes('buildSkillTransferGraph({rows,twin,memory:scenarioMemory,transfer:decisionTransfer})'));
  assert.ok(route.includes('buildClimbCurriculum(twin,scenarioMemory,decisionTransfer,undefined,previousCurriculum,skillTransferGraph)'));
  ...
  assert.ok(repo.includes('scenarioMemory,decisionTransfer,skillTransferGraph,decisionPrincipleEngine,curriculum,generatedAt:now'));
