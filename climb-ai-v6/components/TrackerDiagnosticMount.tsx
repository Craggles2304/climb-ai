...
export function TrackerDiagnosticMount(){
  ...
  return <section className="dash-section" style={{marginBottom:12}}><div className="glass card" style={{padding:16,borderColor:connected?'rgba(214,255,47,.32)':'rgba(255,110,90,.4)'}}><div className="eyebrow">LOCAL COMPANION DIAGNOSTICS</div><div style={{display:'flex',gap:12,alignItems:'center',justifyContent:'space-between',flexWrap:'wrap'}}><div><h3 style={{margin:'6px 0 4px'}}>{headline}</h3><p className="muted" style={{margin:0}}>{detail}</p></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Badge on={connected} label="TRACKER"/><Badge on={Boolean(connected&&t.lcuDetected)} label="LCU"/><Badge on={Boolean(connected&&t.champSelectDetected)} label="CHAMP SELECT"/></div></div>{connected&&t.runtimeVersion&&<p className="muted" style={{fontSize:12,margin:'10px 0 0'}}>Runtime {t.runtimeVersion}</p>}</div></section>;
}
