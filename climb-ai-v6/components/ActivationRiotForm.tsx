...
export function ActivationRiotForm(){
  ...
  return <main className="container section onboarding-page"><Wordmark/><div style={{maxWidth:880,margin:'36px auto 70px',display:'grid',gap:16}}>
    <section className="glass card" style={{padding:'clamp(24px,4vw,44px)'}}><div className="eyebrow">START WITH A REAL GAME</div><h1 style={{fontSize:'clamp(38px,6vw,68px)',lineHeight:.94}}>Connect Riot. See what to fix first.</h1><p className="muted" style={{fontSize:16,lineHeight:1.65}}>We find your player, pull your newest Ranked Solo/Duo game, and give you one clear thing to work on next. No empty dashboard and no ten-point checklist.</p><div className="profile-strip"><div><span>01</span><strong>FIND YOU</strong></div><div><span>02</span><strong>GET A GAME</strong></div><div><span>03</span><strong>YOUR FIRST FOCUS</strong></div></div></section>
    ...
    {error&&<div className="auth-message" role="alert">{error}</div>}<button className="btn primary" disabled={busy||!authenticated||!gameName.trim()} style={{width:'100%',marginTop:16}}>{busy?'CHECKING RIOT + YOUR LATEST GAME…':'SHOW ME WHAT TO FIX'}</button>{busy&&<p className="muted" style={{fontSize:12,textAlign:'center'}}>Finding your account → rank and role → latest ranked game.</p>}{manual&&<button type="button" className="btn secondary" onClick={()=>void continueManual()} style={{width:'100%',marginTop:10}}>CONTINUE WITHOUT RIOT IMPORT</button>}{!authenticated&&<p className="muted">Session expired. <Link className="text-link" href="/login">SIGN IN AGAIN →</Link></p>}</form>
  </div></main>;
}
