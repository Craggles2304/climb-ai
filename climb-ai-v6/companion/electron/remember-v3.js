...
`;
    ...
    section.innerHTML=`
      ...
      <div class="rem4-body">
        ...
        <section class="rem4-spike-strip" aria-label="Power spikes">
          <article><span>EARLY POWER</span><strong id="opRemSpikeEarly">EARLY SPIKE</strong><small id="opRemSpikeEarlyText">Use the first clean power window.</small></article>
          <article><span>MAJOR POWER</span><strong id="opRemSpikeMajor">MAJOR SPIKE</strong><small id="opRemSpikeMajorText">Use your next major item or level window.</small></article>
        </section>
  ...
  function render(state){
    ...
    const spikes=safe(state?.matchup?.plan?.powerSpikes);
    const early=spikes.find(item=>Number(item?.level)===2)||spikes[0]||null;
    const major=spikes.find(item=>Number(item?.level)===6)||spikes.find(item=>Number(item?.level)>2)||spikes[1]||null;
    set('opRemSpikeEarly',early?('LV '+String(early.level||'?')):'EARLY SPIKE');
    set('opRemSpikeEarlyText',upper(clean(early?.fight||early?.label)),'USE THE FIRST CLEAN POWER WINDOW');
    set('opRemSpikeMajor',major?('LV '+String(major.level||'?')):'MAJOR SPIKE');
    set('opRemSpikeMajorText',upper(clean(major?.fight||major?.label)),'USE YOUR NEXT MAJOR ITEM OR LEVEL WINDOW');
