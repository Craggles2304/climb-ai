...
export async function latestLiveReview(userId:string,accountKey:string){
  ...
  const [historyProfile,causalProfile,playerCoachingIdentity,skillTransferGraph,decisionPrincipleEngine,learningVelocity,adaptiveCoachingSession]=await Promise.all([
    ...
    getDecisionPrincipleEngine(userId,session.riot_account_id??null).catch(()=>null),
