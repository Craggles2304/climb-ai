...
export function buildGameReadCalibration(input:{reads?:LiveReadCheckpoint[]|null;points?:StrengthPoint[]|null}):GameReadCalibrationReview{
  ...
  const nextFocus=
    confidenceProfile==='OVERCONFIDENT'?'CALIBRATE BEFORE COMMITTING: A HIGH-CONFIDENCE WRONG READ NEEDS ANOTHER SELF-READ REP BEFORE SUPPORT IS FADED.':
    confidenceProfile==='UNDERCONFIDENT'?'TRUST VERIFIED CORRECT READS MORE: KEEP FREEZING THE READ BEFORE THE PLAN SO CONFIDENCE CAN CATCH UP TO ACCURACY.':
