...
export function isGameMeasurableTask(task:Pick<ILPTask,'metric'|'id'|'evidence'|'progress'>,matches:Match[]){
  if(!matches.length)return false;
  return evaluateMetric(task as ILPTask,matches.slice(0,5)).hasEvidence;
}
...
function candidateForRole(role:Role,matches:Match[]){
  return Object.entries(candidateTasks).filter(([key,candidate])=>{
    if((candidate.roles&& !candidate.roles.includes(role))||!isGameMeasurableMetric(candidate.metric))return false;
    const probe:ILPTask={id:'probe-'+key,accountId:'probe',...candidate,progress:0,status:'ACTIVE',source:'SYSTEM',evidence:[]};
