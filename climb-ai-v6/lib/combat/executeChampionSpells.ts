...
function applyViegoR(
  ...
  // The missing-health bonus is only for the primary champion hit, so the user
  // must explicitly select VIEGO_R_PRIMARY before this component is added.
  ...
  if(!validatedCurrentPatch(ctx.patch)){
    addPartial(
      ...
      `Heartbreaker missing-health coefficients are validated for Data Dragon 16.18.x only; ${ctx.patch} is not auto-carried forward.`,
    );
    return;
  }
  ...
  // The missing-health component is exact, but primary-target Heartbreaker also
  // applies on-hit effects. The current ability event does not yet forward every
  // item/champion on-hit package, so keep the overall primary state PARTIAL.
  removePartials(profile,['VIEGO_R_PRIMARY']);
  addModelled(profile,'VIEGO_R_MISSING_HEALTH');
  addPartial(
    profile,'VIEGO_R_PRIMARY_ON_HIT',
    'Heartbreaker primary missing-health damage is exact, but R-applied item/champion on-hit effects are not yet forwarded through the ability event, so the full primary-target result remains PARTIAL.',
  );
