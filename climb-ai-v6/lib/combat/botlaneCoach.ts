 ...
 * result plus explicit range/access/hit assumptions supplied by the player.
 */
export function buildBotLaneCoachPlan(input:BotLaneCoachInput):BotLaneCoachPlan{
  const {result,focus}=input;
  ...
  const reason=`${damageText} ${firstText} ${focus.reason}`;
  ...
  if(!rerunTriggers.length)
    rerunTriggers.push('Change a key skillshot from HIT to MISS, swap the focus target, or remove auto access to test how fragile this result is.');

  return {
    call,
    headline:headline(call,targetChampion),
    reason,
    target,
    targetChampion,
    rangeDelta,
...
function headline(call:LaneCall,target:string){
