...
function readCache<T>(key:string):T|undefined{
  const hit=cache.get(key);if(!hit)return undefined;
...
function writeCache(key:string,value:unknown,ttl:number){
  ...
  if(cache.size>500){for(const [k] of [...cache.entries()].sort((a,b)=>a[1].expires-b[1].expires).slice(0,100))cache.delete(k)}
}
...
/**
 * A valid server key is enough to enable Riot-backed products. The old setup
 * required a second RIOT_API_ENABLED=true flag, which left sync disabled even
 * when a key had already been configured. RIOT_API_DISABLED=true remains as an
 * explicit emergency kill switch for League and TFT together.
 */
export function riotEnabled():boolean{
  return process.env.RIOT_API_DISABLED!=='true'&&!!process.env.RIOT_API_KEY;
}

const sleep=(ms:number)=>new Promise(r=>setTimeout(r,ms));
export interface RiotFetchOptions{ttl?:keyof typeof TTL;maxRetries?:number}

export async function riotFetch<T>(url:string,opts:RiotFetchOptions={}):Promise<T>{
  const ttl=TTL[opts.ttl||'short'];const cached=readCache<T>(url);if(cached!==undefined)return cached;
  const maxRetries=opts.maxRetries??3;let lastError:RiotApiError|null=null;
  for(let attempt=0;attempt<=maxRetries;attempt++){
    const res=await fetch(url,{headers:{'X-Riot-Token':riotApiKey()},cache:'no-store'});
    if(res.ok){const value=await res.json() as T;writeCache(url,value,ttl);return value}
    ...
    const waitMs=Number.isFinite(retryAfter)&&retryAfter>0?retryAfter*1000:Math.min(8000,2**attempt*500);
    await sleep(waitMs);
  }
  throw lastError;
}
...
export const __cache={size:()=>cache.size,clear:()=>cache.clear()};
